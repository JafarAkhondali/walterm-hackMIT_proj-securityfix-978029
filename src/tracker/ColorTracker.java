...
public class ColorTracker {
	...
	//state variables
	Boolean cursor = false; //true if red is in the image
	Boolean click = false; //true if blue is in the image
	double xPercent = 0;   //the x-coordinate as a percentage of the total width
	double yPercent = 0;   //the y-coordinate as a percentage of the total height
	...
	public void run() throws InterruptedException {
		...
		while(camera.isOpened()){
			...
			/*****CURSOR******/
			//Threshold the image based on the HSV values of the desired color (red for cursor)
			...
			hsvThresh_cursor.convertTo(convert_cursor, CvType.CV_32SC1);  //need image to be 32SC1, not sure why.
			Imgproc.findContours(convert_cursor, contours_cursor, new Mat(), Imgproc.RETR_FLOODFILL, Imgproc.CHAIN_APPROX_SIMPLE);
			
			//Draw all the contours such that they are filled in.
			Mat contourImg_cursor = new Mat(convert_cursor.size(), convert_cursor.type());
			for(int i = 0; i< contours_cursor.size(); i++){
				Imgproc.drawContours(contourImg_cursor, contours_cursor, i, new Scalar(255, 255, 255), -1);
			}
			...
			Imgproc.findContours(contourImg_cursor, contours_cursor, new Mat(), Imgproc.RETR_FLOODFILL, Imgproc.CHAIN_APPROX_SIMPLE);
			if(contours_cursor.size() > 0){
				...
				//If it's less than 20 pixels in area, don't acknowledge that it's there
				if(max > 20){
					this.cursor = true;
					//Get the rectangle that fits around the boundary of the largest contour
					//Use it to figure out the center (x, y) of the contour
					Rect bound = Imgproc.boundingRect(contours_cursor.get(maxCont));
			...
			else{ //if there aren't any contours, the desired color isn't in the image
				this.cursor = false;
			}
			
			/*****CLICK******/
			//Threshold the image based on the HSV values of the desired color (blue for click)
			...
			hsvThresh_click.convertTo(convert_click, CvType.CV_32SC1);  //need image to be 32SC1, not sure why.
			Imgproc.findContours(convert_click, contours_click, new Mat(), Imgproc.RETR_FLOODFILL, Imgproc.CHAIN_APPROX_SIMPLE);
			
			//Draw all the contours such that they are filled in.
			Mat contourImg_click = new Mat(convert_click.size(), convert_click.type());
			for(int i = 0; i< contours_click.size(); i++){
				Imgproc.drawContours(contourImg_click, contours_click, i, new Scalar(255, 255, 255), -1);
			}
			Highgui.imwrite("camera_pic_red_contours.jpg", contourImg_click); //DEBUG
			
			//Find the contours of the filled in mass of previous contours
			Imgproc.findContours(contourImg_click, contours_click, new Mat(), Imgproc.RETR_FLOODFILL, Imgproc.CHAIN_APPROX_SIMPLE);
